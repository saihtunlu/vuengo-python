...
class ChatSessionMessageView(generics.GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        ChatSession_Message = reversed(ChatSessionMessage.objects.filter(
            chat_session=chat_session).order_by('-create_date')[:100:1])
        # paginator = Paginator(ChatSession_Message, 100)
        # pageMessages = paginator.page(2)
        messages = ChatMessageSerializers(ChatSession_Message, many=True).data
        data = {
            'id': chat_session.id, 'uri': chat_session.uri,
            'messages': messages,
            # 'has_next': pageMessages.has_next(),
            # 'has_previous': pageMessages.has_previous(),
            # 'next_page_number': pageMessages.next_page_number(),
        }
